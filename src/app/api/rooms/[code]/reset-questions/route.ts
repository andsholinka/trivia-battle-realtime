import { getRoom, resetQuestions, sanitizeRoom } from "@/lib/rooms";
...
export async function POST(request: Request, context: { params: Promise<{ code: string }> }) {
  ...
  const result = await resetQuestions(room.code, hostId);
  if ("error" in result) {
    return Response.json({ error: result.error }, { status: result.status });
  }

  if (!result.room) {
    return Response.json({ error: "Gagal reset pertanyaan." }, { status: 500 });
  }
